 ...
 */
public class EpidemicRouterF extends ActiveRouter {
     ...
     */
    public EpidemicRouterF(Settings s) {
        super(s);
     ...
     */
    protected EpidemicRouterF(EpidemicRouterF r) {
        super(r);
        this.fcl = r.fcl;
...
// method ini berisi daftar pesan yang sudah sampai ke tujuan akhir yang diketahui oleh setiap node
    ...
    public void exchangeAckedList() {
        ...
        for (Connection con : connections) {
            ...
            for (String ackedMessage : otherAckedMessages) {// untuk  setiap ID pesan yang sudah sampai ketujuan  yng diketahui oleh otherRouter.
            ...
            for (String ackedMessage : myAckedMessages) { // untuk  setiap ID pesan yang sudah sampai ketujuan  yng diketahui oleh this node
    ...
    public void deleteAckedList() {
        ...
        List<String> messagesToDelete = new ArrayList<>();// daftar pesan untuk dihapus
...
// deliverable message adalah pesan yang tujuannya node yang terhubung
    ...
    protected Connection tryAllMessagesToAllConnections() {
        ...
        for (Connection c : connections) {
            ...
            EpidemicRouterF peer = (EpidemicRouterF) otherHost.getRouter();
            for (Message m : mergeMsg) {
                    if (!peer.hasMessage(m.getId()) && m.getSize() < peer.getFreeBufferSize()) {
                        temp.add(m);
                        //Kondisi ini memeriksa apakah node tujuan (peer) sudah memiliki pesan dengan ID tertentu 
                    }// apakah ukuran pesan (m.getSize()) lebih kecil dari ruang buffer yang tersedia di node tujuan
                }
            }
    ...
    //        if (connections.size() == 0 || this.getNrofMessages() == 0) {
    //            return null;
    //        }// jika tidak ada koneksi atau tidak ada pesan maka return null
    //
    //        List<Message> mergeMsg = getMergedSortMessage();
    //        //Memanggil metode getMergedSortMessage untuk mendapatkan daftar pesan yang 
    //        //telah digabungkan dari semua koneksi dan diurutkan berdasarkan prioritas.
    //
    //        return tryMessagesToConnections(mergeMsg, connections);
    //        //kirimkan pesan-pesan tersebut ke semua koneksi yang tersedia
    //   
    ...
    protected List<Message> getMergedSortMessage() {
        ...
        for (Connection c : getConnections()) { // untuk setiap koneksi yang tersedia
            ...
            EpidemicRouterF peer = (EpidemicRouterF) other.getRouter();
    ...
    @Override
    public EpidemicRouterF replicate() {
        return new EpidemicRouterF(this);
    }
